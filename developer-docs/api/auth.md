...
外部认证当前实现的 provider kind 是 `oidc` 和 `generic_oauth2`，管理端通过 `/admin/external-auth/*` 配置。匿名登录页先调用 `GET /auth/external-auth/providers` 读取启用中的 provider：
...
`oidc` driver 使用 discovery、PKCE、nonce 和 ID Token 校验；`generic_oauth2` driver 使用手动 endpoint、PKCE、token exchange 和 UserInfo claim 映射。更完整的模块说明见 [外部认证模块](../external-auth.md)。
