...
| Driver 注册表 | `src/external_auth/registry.rs` | 注册 `oidc` 和 `generic_oauth2` |
...
- 管理端 provider API：`developer-docs/api/admin.md#外部认证提供商`
- 登录端外部认证 API：`developer-docs/api/auth.md#外部认证`
...
- GitHub 通过 `/user` 不能可靠获得 verified primary email；专用 GitHub provider 或 `/user/emails` 支持见 <https://github.com/AptS-1547/AsterDrive/issues/235>。
