...
- `get_range` / `supports_efficient_range`：默认基于 `get_stream` 丢弃前缀字节，支持原生 Range 或可 seek 的驱动应覆盖
- `copy_object`：默认 get + put，支持 server-side copy 的驱动应覆盖
- `readiness_check`：默认直接成功；只应做本地低成本检查，不许在默认实现里加远端 I/O
- `capacity_info`：默认返回 `Unsupported` 错误，不允许静默猜测或 panic

只有所有存储后端都必须支持的能力，才允许直接加进核心 trait。

### 可选能力走 `extensions()` bundle

并非每个驱动都能做 presigned URL、路径列举、流式上传或 provider 原生缩略图。这些可选能力各自是独立的 extension trait，驱动通过唯一的 `extensions()` 方法把它们打包成 `StorageDriverExtensions` 返回：

| 字段 | 能力 |
| --- | --- |
| `presigned` | 临时下载 / 直传 URL |
| `list` | 路径列举与流式扫描（面向维护 / 审计任务） |
| `stream_upload` | reader / 本地文件直接写入 |
| `provider_resumable` | provider-native resumable session（如 Microsoft Graph upload session） |
| `local_path` | 暴露本机真实路径给受控外部命令 |
| `native_thumbnail` | 存储侧原生缩略图 |
| `native_media_metadata` | 存储侧原生媒体元数据解析 |
| `multipart` | 对象存储 multipart 编排 |

调用方写法是 `driver.extensions().presigned` 这样的能力查询，拿到 `Option<&dyn Trait>`，`None` 表示当前驱动不支持。

这里曾经有过“每个能力一个 `as_xxx()` 方法”的写法，后来统一收敛成单个 `extensions()`，原因很实际：每加一个能力，核心 trait 和层级里每个 decorator / wrapper 都得跟着加一个转发方法。bundle 之后，装饰器把内部驱动的 `StorageDriverExtensions` 整体透传即可，新能力只需要动 `extensions.rs` 和能力双方的驱动。

`MetricsStorageDriver` 是这个模式最直接的受益者：它的 `extensions()` 先取内层 bundle，只把 `multipart` 替换成带操作级指标的 `MetricsMultipartStorageDriver` 包装，其余能力原样转发。
...
`audit_storage_objects()` 不直接假定底层一定是本地文件系统或某种固定 S3 SDK，而是复用 `driver.extensions().list`（`ListStorageDriver`）暴露出来的遍历能力，大规模扫描走 `scan_paths` 逐条 visitor 回调，不一次性拉全量 key。
...
也就是说，`doctor` 的对象审计依赖的是“驱动可枚举”这个可选能力，而不是某个具体存储后端的实现细节；不暴露 `list` 能力的驱动只会被跳过，不影响其他审计项。
...
页面只负责收集当前工作空间里的选择，并把文件 / 文件夹 ID 与展示所需的名称、大小交给下载层。页面本地列表可能因为分页、搜索刷新或项目过期而落后于用户选择；如果本地映射数量和请求 ID 数量不一致，页面必须把原始 ID 交给后端 ZIP 下载接口重新解析，不能静默丢掉缺失项后继续下载。

`downloadCoordinator` 负责浏览器内下载的协议细节和生命周期，包括：

- 通过 workspace-aware file service 构造认证下载路径
- 代理流式下载、进度、取消、失败重试和内存回退
- 目录写入能力探测与文件夹递归遍历
- 文件 / 文件夹分页游标推进

分页循环除了检查 `next_*_cursor`，还必须检查当前页是否真的返回项目。后端如果返回空页和陈旧游标，协调器应立即终止，避免重复请求或死循环。

`DownloadCenter` 只负责下载方式选择和任务展示，不直接拼文件下载路径，也不自行决定认证策略。单文件“使用浏览器默认下载”走 coordinator 的统一认证文件下载入口；多选“使用浏览器下载 ZIP”走 batch service 的后端打包流。代理文件、代理 ZIP 和下载到本地目录则由 coordinator 创建可观察的浏览器下载任务。

### 后端能力保持权威

匿名启动接口 `GET /api/v1/public/frontend-config` 的 `downloads` 字段公开登录用户和分享场景的 ZIP 下载开关。前端配置 store 缓存并消费这些字段；开关关闭或启动配置不可用时，前端保守隐藏 ZIP 方式。对应后端端点仍会重新检查运行时配置，因此前端能力判断只是展示逻辑，不替代服务端授权和功能开关校验。

### 统一活动区与布局避让

上传面板和下载中心都通过 `BottomRightActivityPortal` 进入同一个右下角 shell。shell 负责统一宽度、边框、层级和分区，`transferActivityStore` 负责当前展开的是上传还是下载；上传 manager 不保存重复的展开状态。

这个 shell 视觉上固定在右下角，但不能覆盖文件列表最后几行。`BottomRightActivityShell` 用 `ResizeObserver` 测量实际高度并写入 `--bottom-right-activity-shell-height`，文件浏览、搜索、分类、分享和回收站内容区通过统一 padding 常量为它预留空间。新增会显示右下角活动区的页面时，也必须接入同一避让常量，而不是再做一个独立浮层。
