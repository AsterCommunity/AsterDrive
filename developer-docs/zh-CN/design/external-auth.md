...
6. 用户在身份提供商授权后按 provider 持久化的 `callback_mode` 回调统一 `/auth/external-auth/callback` 或兼容旧 URI `/auth/external-auth/{kind}/{provider}/callback`。旧 URI 仅在迁移期保留，TODO：1.0.0 删除。
