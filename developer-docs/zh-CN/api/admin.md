...
- `options` 当前承载策略级行为：
  ...
  - S3 SigV4 签名 region：`s3_region`。该字段为可选字符串，空值或纯空白会规范化为未配置，运行时使用 `auto`；自定义 S3-compatible endpoint 若要求固定 region，应填写服务商提供的值。草稿连接测试和保存后的 S3 runtime driver 使用同一个 region
