...
- `options.object_storage_upload_strategy`：控制 S3-compatible、Azure Blob、Tencent COS、Huawei OBS 这类对象存储 connector 的传输策略
