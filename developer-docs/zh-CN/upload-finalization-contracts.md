...
- Init 调用 `create_frontend_upload_session(object_temp_key)` 创建 provider session，`object_temp_key = files/{upload_id}`。upload URL 本身等价于写凭据，因此加密存入 `upload_sessions.provider_session_ciphertext`：密钥用 `auth.storage_credential_secret_key`，AAD 为 `upload_session:{upload_id}:provider_resumable`。
