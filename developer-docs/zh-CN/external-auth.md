...
| Driver 注册表 | `src/external_auth/registry.rs` | 注册 `oidc`、`generic_oauth2`、`github` 和 `google` |
...
- Microsoft 当前可通过通用 OIDC 手动配置；手动配置应优先使用具体 tenant issuer，因为当前 OIDC driver 会严格要求 ID Token issuer 等于 provider `issuer_url`。Microsoft 专用预设见 <https://github.com/AptS-1547/AsterDrive/issues/263>。
