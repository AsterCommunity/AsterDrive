...
- A field descriptor's `update_behavior` declares its edit lifecycle: `mutable` remains editable, `create_only` is writable only during creation, and `set_once` may fill a legacy empty value once before becoming read-only. Generic frontend rendering consumes this metadata instead of inferring behavior from connector IDs, field names, or data sources; backend services independently enforce the same invariant so direct API calls cannot bypass it.
