...
This endpoint does more than a database ping. It checks the database first; the cluster profile also checks the active Redis cache. It then continues according to node mode:
...
- unhealthy cluster Redis cache: `503` with `Shared cache unavailable`
