...
This page keeps the most important admin endpoint groups. For usage-oriented admin-console guidance, see [Admin Console](https://drive.docs.astercosm.com/en/admin/).
...
- `options` carries policy-level behavior:
  ...
  - The Tencent COS descriptor declares `custom_download_base_url` and exposes the connector-owned optional `download_base_url` field for an HTTPS browser-download delivery root. The runtime driver builds the final object path and COS Q-Sign against that host; backend I/O, uploads, cleanup, CORS, and bucket administration continue to use the official COS endpoint.
  ...
  - Huawei OBS fields: regional or custom-domain `endpoint`, `bucket`, optional `obs_region`, and `obs_addressing_mode`; OBS uses marker-based ListObjects pagination and the same object-storage upload/download strategies as other S3-shaped connectors
...
- `driver_type = "tencent_cos"` uses the S3-compatible object path for normal reads and writes and validates Tencent COS endpoint shape. Optional `download_base_url` accepts an HTTPS delivery domain with a controlled path prefix and rejects userinfo, non-default ports, query, fragment, backslashes, and traversal; the connector can also expose COS CI storage-native thumbnail / image-preview / media-metadata capabilities when the policy opts in
...
- `driver_type = "huawei_obs"` reuses AWS S3 SDK object, streaming, and multipart serialization while applying native Huawei OBS signing; virtual-hosted and custom-domain addressing, marker-based listing, normal requests, and generated presigning are validated separately
