...
Target-capacity admission is a fast-fail snapshot, not a cross-request space reservation. The final workspace quota remains protected by the completion transaction's SQL CAS. Capacity observation is demand-driven rather than periodically scanned: concurrent probes for one policy are coalesced, total probe concurrency is bounded, and each driver owns its freshness and timeout policy. Local filesystem observations are fresh for two seconds and may serve sufficient results for up to 30 seconds; OneDrive and Remote observations are fresh for 30 seconds and may serve sufficient results for up to five minutes with stale-while-revalidate. Stale insufficient or unavailable results are refreshed before rejection. Local probes use a fixed two-second timeout; OneDrive and Remote default to ten seconds and expose a two-to-30-second connector setting. Temporary failures use short driver-owned negative caching, and a shared probe survives cancellation of its initiating request.

Sessions requiring `OffsetStaging` / `StreamStaging` serialize temporary-disk admission against `upload_temp_dir` before Init returns and physically preallocate the complete file. Allocation must leave `server.upload_temp_min_free_bytes` free (256 MiB by default); insufficient space returns `upload.staging_capacity_insufficient` (HTTP 507) without leaving a session or temporary directory. After restart, the first staged Init, Chunk PUT, or Complete restores missing physical allocation for active sessions. Completion, cancellation, and expiry cleanup release reservations by deleting their temporary directories.
