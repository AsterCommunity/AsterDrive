...
Capacity admission is a fast-fail snapshot, not a cross-request space reservation. The final workspace quota remains protected by the completion transaction's SQL CAS. Capacity observation is demand-driven rather than periodically scanned: concurrent probes for one policy are coalesced, total probe concurrency is bounded, and each probe has a two-second limit. Reliable observations are reused for a two-second fresh window; a sufficient observation remains usable for up to 30 seconds with one stale-while-revalidate refresh, while stale insufficient or unavailable results must be refreshed before rejection. Temporary failures are cached for only 250 milliseconds, and a shared probe survives cancellation of its initiating request. Physical staged-space reservation remains tracked by Issue #593.
