...
AsterDrive 文档站提供可直接导入的 Grafana dashboard JSON。官方文档站和自托管 VitePress 文档站都会在站点根路径下分发这个文件：
...
```text
https://asterdrive.docs.esap.cc/grafana/asterdrive-overview.json
```
...
`storage_driver_operations_total{status="failure",kind="not_found"}` 建议单独观察。缩略图、缓存或对象探测场景里，`not_found` 可能只是预期的 miss；告警规则更适合优先关注 `kind!="not_found"` 的硬失败。
