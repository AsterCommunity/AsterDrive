---
description: AsterDrive 单实例 Docker 部署场景页，覆盖镜像用户与目录权限、试跑、长期 config.toml 管理、Compose 示例、aria2 链接导入、上线验收和升级维护入口。
title: "单实例 Docker 部署"
---

:::tip[适用场景]
NAS、单机、小团队，或者已经在用容器编排的单实例部署。10 分钟能跑起来。
**正式上线时**前面一定要接反向代理处理 HTTPS——`3000` 端口不要直接对公网开。
:::

:::note[不适用场景]

- 需要多个 Primary 共享数据面 → [负载均衡与多实例](/deploy/multi-instance/)
- 用 Kubernetes 编排 → [Kubernetes 部署](/deploy/kubernetes/)
- 想把这台跑成远程存储节点 → [Docker 部署从节点](/deploy/follower-node/)
...
- Docker 或兼容容器运行时
- 一块可以长期保留的数据目录（bind mount 推荐，备份和迁移更直观）
- 正式上线时：一个已解析的域名和反向代理（见 [反向代理](/deploy/reverse-proxy/)）
...
如果你想用 Docker 把另一台 AsterDrive 接成远程节点，不再建议沿用旧流程手动 `docker exec` 执行 `node enroll`，直接看 [Docker 部署从节点](/deploy/follower-node/)。
:::
...
当 aria2 以不同的 OS 用户运行时，AsterDrive 必须允许这个外部写入者在每个任务的 `token_dir` 下创建下载临时文件。`allow_external_aria2_writer_chain` 的兼容路径会把每个任务的目录设为可写，同时让共享的 tasks 父目录仅可穿越。这在单租户、临时卷不与不可信本地用户共享的隔离 Compose 部署里是可以接受的。更稳妥的生产做法是：让两个进程跑在同一个 UID 下、分配共享组并使用 `0o770`，或者为 aria2 用户在 `token_dir` 上配置 POSIX ACL。
...
完整配置、安全边界和常见问题见 [离线下载](/admin/offline-download/)。
...
- 如果这台实例实际要跑成 `follower`，是否已经按 [Docker 部署从节点](/deploy/follower-node/) 配好长期 `start_mode`、一次性 bootstrap ENV，并在主控节点创建远程存储目标、由 remote 策略显式绑定

正式上线前的完整清单见 [生产上线检查清单](/ops/launch-checklist/)。
...
- 升级：见下面"升级"一节，完整版本迁移说明见 [升级与版本迁移](/ops/upgrade/)
- 备份：见 [备份与恢复](/ops/backup/)
- 监控：见 [监控与 Grafana](/ops/monitoring/)
- 排障：容器起不来、上传失败等问题按 [故障排查](/ops/troubleshooting/) 定位
