---
description: AsterDrive 单实例 systemd 部署场景页，覆盖运行目录、可执行文件、config.toml、service 文件、首次验收、常用环境变量和 HTTPS 反向代理衔接。
title: "单实例 systemd 部署"
---
...
- 已经有容器环境、想要更快的交付方式 → [单实例 Docker 部署](/deploy/docker/)
- 需要多个 Primary 或 Kubernetes 编排 → [负载均衡与多实例](/deploy/multi-instance/)

:::

## 前置条件

- 一台长期运行的 Linux 主机和 sudo 权限
- 从 release 获取的 `aster_drive` 可执行文件
...
SQLite、本地存储目录、头像目录和你自定义的本地 `local` 存储根目录，都应该纳入同一套备份策略。见 [备份与恢复](/ops/backup/)。
...
确认接入成功后，建议删掉这两个 `ASTER_BOOTSTRAP_REMOTE_*`，只保留长期运行需要的 `ASTER__...` 覆盖项。完整流程见 [远程节点](/admin/follower-nodes/)。

## 9. HTTPS 与域名

systemd 只负责把服务拉起来。
如果你要提供 HTTPS、域名、WebDAV 客户端访问，或者外部 Office / WOPI 打开方式，还需要在前面加一层反向代理。代理层负责 TLS、HSTS、上传限制和长连接透传，同时保留 AsterDrive 返回的页面 CSP，见 [反向代理部署](/deploy/reverse-proxy/)。

## 日常维护入口

- 备份：见 [备份与恢复](/ops/backup/)
- 升级：替换可执行文件后 `systemctl restart asterdrive`，完整版本迁移说明见 [升级与版本迁移](/ops/upgrade/)
- 监控：见 [监控与 Grafana](/ops/monitoring/)
- 排障：服务起不来、端口不通等问题按 [故障排查](/ops/troubleshooting/) 定位
