...
SQLite 的好处是：不需要额外部署数据库服务、数据库本身是单文件，试用门槛低到几乎为零。停掉 AsterDrive 后，数据库和本地持久化目录也比较容易一起做冷备；服务运行时不要只复制数据库文件，更不要只备份数据库而漏掉文件对象和 `config.toml`。完整边界见 [备份与恢复](/deployment/backup/)。
