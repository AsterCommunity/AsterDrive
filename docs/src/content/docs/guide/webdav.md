...
- collection `Depth: infinity` 锁对子资源的 token 授权还存在客户端兼容差异，由 [GitHub #425](https://github.com/AsterCommunity/AsterDrive/issues/425) 跟踪。依赖目录级递归锁的工作流应先用实际客户端验证；普通文件级锁不等于这一边界。
