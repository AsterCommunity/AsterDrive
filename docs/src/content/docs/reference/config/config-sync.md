...
本页只解释通知和恢复机制；共享依赖、存储、上传、健康检查和负载均衡限制见[负载均衡与多实例](/deploy/multi-instance/)。
:::
...
上线前也应把 Redis 可用性和所有实例的 topic 一致性加入 [生产上线检查](/ops/launch-checklist/)，并按[负载均衡与多实例](/deploy/multi-instance/#上线验收)完成跨实例验收。
