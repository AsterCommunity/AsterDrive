---
description: "AsterDrive config.toml 数据库配置字段：SQLite / PostgreSQL / MySQL 连接串、连接池参数和后端选择建议。"
title: "数据库配置"
---
...
启动时的自动迁移只负责"更新目标库 schema"，不会把旧数据库的业务数据搬过去。要走这种场景，先用 [运维 CLI](/ops/cli/) 的 `database-migrate` 把数据迁过去，再切换生产实例。
:::
