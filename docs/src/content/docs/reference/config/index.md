...
- **反向代理 / 对象存储自己的配置** —— HTTPS、大文件上传、WebDAV 透传、S3 / Azure Blob / COS / OBS 直传
...
| 你想做什么 | 去哪里改 |
| --- | --- |
| 改监听地址、端口、线程数、临时目录、primary / follower 模式 | [服务器](/reference/config/server/) |
| 改数据库地址、连接池、启动重试 | [数据库](/reference/config/database/) |
| 固定登录签名密钥、MFA 加密密钥、第一次纯 HTTP 引导 | [登录与会话](/reference/config/auth/) |
| 公开站点地址、品牌、注册、Cookie、Token、调度、分享流播放、压缩包预览、回收站、版本、配额、WOPI、WebDAV、审计 | [系统设置](/reference/config/runtime/) |
| 接 OIDC / 通用 OAuth2 / SSO 外部登录，管理外部身份提供商 | [外部认证](/reference/config/external-auth/) / [登录与会话](/reference/config/auth/) |
| 配 SMTP、发测试邮件、改邮件模版 | [邮件](/admin/mail/) |
| 配链接导入、内置下载器或 aria2 离线下载 | [离线下载](/admin/offline-download/) |
...
| 多实例间同步后台系统设置和 config CLI 修改 | [配置同步](/reference/config/config-sync/) |
...
| 运维 CLI 参数 | `ASTER_CLI_DATABASE_URL`、`ASTER_CLI_OUTPUT_FORMAT` | 脚本里不想每次写长参数时使用，见 [运维 CLI](/ops/cli/) |
...
| 分区 | 作用 |
| --- | --- |
| [deployment](/reference/config/deployment/) | 单实例或多 primary 集群部署声明，以及共享依赖检查 |
| [server](/reference/config/server/) | 监听地址、端口、线程数、临时目录、节点模式、follower 接收根目录 |
| [database](/reference/config/database/) | 数据库连接、连接池、启动重试 |
| [auth](/reference/config/auth/) | 登录签名密钥、MFA 加密密钥、首次纯 HTTP 引导 |
| [cache](/reference/config/cache/) | 内存缓存 / Redis |
| [config_sync](/reference/config/config-sync/) | 多实例运行时配置 reload 通知，默认关闭 |
| [logging](/reference/config/logging/) | 日志级别、格式、输出文件、轮转 |
| [webdav](/reference/config/webdav/) | WebDAV 路径前缀、上传体积硬上限 |
| `[network_trust]` | 受信任的反向代理地址，影响真实客户端 IP 判定 |
| [rate_limit](/reference/config/rate-limit/) | 登录、公开分享和一般访问的限流规则 |
...
- 准备开放外部认证前，先填对 `公开站点地址`，再到 `管理 -> 外部认证` 复制重定向 URI；详细步骤见 [外部认证](/reference/config/external-auth/)
...
如果后台暂时进不去，或者你想在停机窗口里离线检查、校验或批量写入系统设置，走 [运维 CLI](/ops/cli/)。
...
详情见 [存储策略](/admin/storage-policies/)。
