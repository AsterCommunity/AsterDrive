---
description: "AsterDrive 运行架构：primary / follower 节点、组件关系、多 primary 资源边界、上传下载数据流、配置两层边界和排障定位入口。"
title: "运行架构"
---
...
| 跨实例控制面 | Redis cache、config sync Pub/Sub、storage event notification、数据库 tunnel owner directory | 只传递失效、刷新、owner discovery 等小消息；断线恢复后回到权威数据库 reconcile |
...
如果你不知道从哪开始，先跑 [运维 CLI](/ops/cli/) 里的 `doctor`，再按 [故障排查](/ops/troubleshooting/) 对照具体症状。
