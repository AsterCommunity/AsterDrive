...
The example uses `/health` for startup and liveness probes, and `/health/ready` for readiness. Readiness checks the shared database, the real Redis cache, and the default storage policy. Pods that fail readiness are removed from the public Service.

A fresh cluster database has no default storage policy, so `/health/ready` initially returns `503`. You can still create the first administrator through `/api/v1/auth/setup` on either Primary; that administrator temporarily has no policy group. After an administrator creates a shared storage policy reachable by every Primary and marks it as default, AsterDrive atomically creates the default policy group, assigns administrators that do not yet have a policy group, and notifies the other Primaries through Redis to reload. Normal registration, invitation acceptance, and administrator-created users still require an existing default policy group.
