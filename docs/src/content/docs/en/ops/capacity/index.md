...
AsterDrive's real capacity depends on database backend, storage policy, filename length, version count, search indexes, background tasks, upload mode, and concurrent access. Before production launch, still run [Performance Benchmarking and Load Testing](/en/ops/capacity/benchmarking/) in your own environment.
:::
...
Before raising background task concurrency, validate with mixed workloads that the foreground path is not slowed down:
...
When tuning, change one lane at a time and keep before/after summaries. Focus on:

- Foreground REST download / upload / WebDAV GET p95, p99, and error rate.
- Whether `aster_mixed_*_task_backlog` keeps accumulating, especially `retry`.
- `background_tasks_pending`, `background_task_retries_total`, DB query latency, and storage operation latency.
- Whether `data/.tmp`, `data/.uploads`, RSS, and CPU keep growing.

If background tasks finish faster but foreground WebDAV or upload p99 rises noticeably, that configuration is not suitable for production. For small teams, it is better to let background tasks queue than to make the file read/write path jitter in ways users can feel — configurations that "look like higher throughput" will come back to bite you.
