---
description: "AsterDrive production launch checklist: full acceptance for data directories, access entry, database, storage policies, monitoring, backup, upgrade readiness, and rollback plans."
...
| --- | --- | --- |
| Data directory | `config.toml`, database, local storage directories, and avatar directory are all persisted | [Backup and Restore](/en/ops/backup/) |
| Access entry | Users access through HTTPS, and the reverse proxy preserves required request and response headers | [Reverse Proxy](/en/deploy/reverse-proxy/) |
| Public Site URL | `Admin -> System Settings -> Site Configuration -> Public Site URL` is a real HTTPS origin | [System Settings](/en/reference/config/runtime/site/) |
| Login security | `jwt_secret` is fixed, and HTTPS-only Cookies are enabled | [Login and Sessions](/en/reference/config/auth/) |
| Storage route | Default storage policy, default policy group, and user/team bindings match expectations | [Storage Policies](/en/admin/storage-policies/) |
| Mail | Registration activation, password reset, and email rebinding messages can be delivered | [Mail](/en/admin/mail/) |
| Load balancing and multi-instance | Primaries share the database, Redis, static secrets, and a supported storage data plane, and cross-instance switching has been verified | [Load Balancing and Multi-Instance Deployments](/en/deploy/multi-instance/) |
| Monitoring | If Prometheus is needed, the `metrics` feature is compiled as needed, and `/health/metrics` access sources are restricted | [Monitoring and Grafana](/en/ops/monitoring/) |
| Backup | At least one backup has been completed, and the restore sequence is known | [Backup and Restore](/en/ops/backup/) |
| Upgrade | The current version source is known, and release notes have been reviewed before upgrade | [Upgrade and Version Migration](/en/ops/upgrade/) |
...
The full integration flow is in [Online Preview and WOPI](/en/admin/preview-processing/).
...
Grafana dashboard and local Prometheus + Grafana examples are in [Monitoring and Grafana](/en/ops/monitoring/).
...
See [Load Balancing and Multi-Instance Deployments](/en/deploy/multi-instance/) for the complete limits and acceptance steps, and [Configuration Synchronization](/en/reference/config/config-sync/) for notification failure behavior.
