...
The initial state depends on the deployment profile:

| Profile | First-start behavior |
| --- | --- |
| `single` | Creates the local policy `Local Default`, then creates `Default Policy Group` pointing to it |
| `cluster` | Skips the local policy and policy group while waiting for an administrator to create storage reachable by every Primary |

With an unchanged `single` setup, new users bind to the default policy group and upload through the default local policy. In a `cluster`, creating the first administrator moves the system to `needs_storage`; when that administrator makes the first shared policy the default, AsterDrive atomically creates the default policy group, assigns administrators that have no group, and moves to `ready`.
...
The upload mode can be `server_relay` or `frontend_direct`. `server_relay` is the default retained for existing-policy compatibility and sends files through AsterDrive. `frontend_direct` lets the browser upload directly to Microsoft Graph, reducing bandwidth on the AsterDrive node. The download mode can be selected separately as server relay or Microsoft Graph direct download; new OneDrive policies default to direct download, while existing policies keep their saved choice. Direct download performs AsterDrive permission checks first and then lets the browser fetch the file from Microsoft. OneDrive also offers a download filename choice: the default prefers the filename stored in OneDrive, while strict AsterDrive filenames use relay streaming when the names differ. New uploads keep their original filenames, and same-name files are stored independently. Microsoft provides the cross-origin support required for direct upload, so no additional AsterDrive setting is needed. See the [OneDrive storage policy tutorial](/en/storage/onedrive/) for the full flow.
