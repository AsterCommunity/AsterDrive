---
description: "AsterDrive quick start: run the service with Docker in 10 minutes, create the first administrator, configure default storage, and validate upload, share, recycle bin, and WebDAV."
...
After production launch, if your data keeps growing, you can switch to PostgreSQL later. AsterDrive ships with a [cross-database migration tool](/en/ops/cli/) so the initial database choice does not trap you.
</details>
...
- Whether the current default policy still fits the deployment: a single instance may keep local storage or move to S3 / MinIO / Azure Blob / Tencent COS / OneDrive / SFTP / a follower node; if using follower nodes, whether the policy explicitly binds an applied remote storage target
...
If you also want an offline command-line check, run `doctor` from [Operations CLI](/en/ops/cli/).

## What Next?

After the basic flow works, you may want to do these next:

- Launch for real, add HTTPS, and configure a reverse proxy -> [Deployment Overview](/en/deploy/) / [Reverse Proxy](/en/deploy/reverse-proxy/)
- Build a primary + follower-node deployment -> [Follower Nodes](/en/admin/follower-nodes/)
- Learn what daily use looks like -> [User Manual](/en/using/)
- Go deeper into the admin console -> [Admin Console](/en/admin/)
- Connect Office online editing -> [File Editing](/en/using/preview-editing/)
- Hit a problem -> [Error Code Handling](/en/reference/errors/) / [Troubleshooting](/en/ops/troubleshooting/)
