...
| Driver type | Built-in connectors use namespaced IDs, for example `asterdrive.storage.alibaba_oss` and `asterdrive.storage.huawei_obs` |
| Connection info | Local directory / S3 endpoint, bucket, keys / OSS public endpoint, optional server-side endpoint, region, bucket, CNAME, keys / Azure Blob endpoint, container, account keys / COS endpoint, bucket, keys / OBS endpoint, bucket, region, addressing mode, signing mode, and keys / OneDrive Microsoft Graph target and authorization / SFTP endpoint, SSH credentials, host key fingerprint / bound remote node |
...
| Additional options | Local content dedup, S3 / OSS / Azure Blob / COS / OBS upload and download modes, S3 path-style access, OSS CNAME, OBS region / addressing / signing mode, OneDrive drive targeting, SFTP host key fingerprint, remote upload/download modes, storage-native processing switch, etc. |
...
- **Test a draft config**: probe with current form values before saving; for static-credential backends like S3, Alibaba Cloud OSS, Azure Blob, Tencent COS, and Huawei OBS, leaving the secret field empty reuses the saved credential.
...
| `s3` / `alibaba_oss` / `tencent_cos` / `asterdrive.storage.huawei_obs` | Returns "unsupported"; these object-storage APIs have no uniform, reliable bucket free-space interface |
