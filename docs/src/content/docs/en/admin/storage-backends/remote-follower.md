---
description: "Full walkthrough for using an enrolled AsterDrive follower node as a remote storage policy backend: remote storage target, remote policy, policy group rules, user or team binding, and upload/download validation."
...
If you have not connected the follower node to the primary node yet, start with [Follower Nodes](/en/admin/follower-nodes/).  
If you run follower nodes with Docker, start with [Deploy a Follower Node with Docker](/en/deploy/follower-node/).
...
The Follower itself is not a second Primary and does not provide an independent control plane. The controlling side may remain one Primary or run multiple Primaries under the [cluster load-balancing contract](/en/deploy/multi-instance/); both topologies use the Follower as a remote storage data plane.
...
To let public users access these files, either give the follower a public HTTPS address, or set the remote policy upload/download mode to `relay_stream` so the primary relays the traffic. For topology trade-offs, see [Follower Node Network Topologies](/en/deploy/follower-node/network/).
:::
