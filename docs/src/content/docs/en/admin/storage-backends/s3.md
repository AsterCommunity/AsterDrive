---
description: "Full walkthrough for storing AsterDrive files on S3 / MinIO / R2 object storage: bucket preparation, storage policy, policy groups, user binding, presigned-upload CORS, and validation."
...
### Choosing the signing region

The admin `Region` field maps to storage policy `options.s3_region` and is used for S3 SigV4 request signing. It is independent from Endpoint and Bucket, and AsterDrive does not infer a region from an arbitrary custom endpoint.

- Leading and trailing whitespace is trimmed. An empty or whitespace-only value uses `auto`, preserving compatibility with existing policies that do not configure a region.
- A non-empty value must contain 1–128 printable ASCII characters and must not contain whitespace or `/`. Invalid values are rejected as `BadRequest` (HTTP 400 with response code `bad_request`).
- For AWS S3, enter the bucket's actual region, such as `us-east-1`.
- Cloudflare R2 normally uses `auto`.
- For MinIO, RustFS, and other S3-compatible services, use the value required by the provider, such as `us-east-1` or a provider-specific region.
- Connection tests and saved runtime requests use the same signing region. Test the connection again after changing it.
...
:::tip[Huawei Cloud OBS CORS fields]
Huawei Cloud OBS uses its native object-storage request protocol, so do not mechanically copy the `x-amz-*` request headers from the generic example. OBS presigned uploads need at least `Content-Type` in the allowed request headers; multipart uploads and Range downloads need response headers such as `ETag`, `Content-Length`, `Content-Range`, and `Accept-Ranges` in the exposed-header field. If the OBS console returns `403` for an `OPTIONS` preflight, check that the origin, `PUT` method, and `Content-Type` match the same CORS rule.
:::
...
For the complete backup boundary, see [Backup and Restore](/en/ops/backup/).
