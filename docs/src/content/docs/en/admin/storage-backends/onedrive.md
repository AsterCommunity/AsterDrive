...
Large files are not assembled in a complete Pod-local staging file first. AsterDrive creates a Microsoft Graph upload session during initialization, streams each browser chunk as one sequential `Content-Range` request, and stores the encrypted provider-session state in the shared database. In a multi-Primary deployment, initialization, later chunks, progress checks, and completion can be handled by different Primaries without sticky sessions or a shared temporary directory.

The Graph upload URL is a preauthenticated credential. In `server_relay` mode it is never returned to the browser; the browser only calls AsterDrive's authenticated chunk endpoint. The backend also limits the session to one chunk at a time and queries provider progress before retrying or advancing after an ambiguous Graph response.
...
Microsoft access and refresh tokens always stay on the AsterDrive server and are never sent to the browser. Both modes support resuming interrupted uploads, while canceled or expired uploads are cleaned up automatically. Only `frontend_direct` gives the temporary Graph upload URL to the authenticated browser.
...
Credential encryption depends on `auth.storage_credential_secret_key`. Preserve this setting when backing up or migrating AsterDrive. See [Authentication & Session - `storage_credential_secret_key`](/en/reference/config/auth/#storage_credential_secret_key).
