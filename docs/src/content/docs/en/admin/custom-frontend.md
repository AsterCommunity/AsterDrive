---
description: "AsterDrive custom frontend override mechanism: asset loading order, index.html placeholders, using custom configuration as a global-variable persistence layer, and CSP and security limits."
...
The operations CLI also supports custom configuration. `list` / `get` / `set` / `delete` / `validate` / `export` / `import` all work. See [Operations CLI](/en/ops/cli/).
