---
description: "Storage capability matrix for the nine backends: browser direct upload / download, capacity observation, storage-native processing, credentials at rest, and the authoritative relay_stream vs presigned comparison."
...
| `asterdrive.storage.huawei_obs` | `presigned` upload + download (requires OBS CORS) | Not supported | Not supported | AES-256-GCM encrypted |
