---
description: "AsterDrive config.toml server fields: listen address, port, node mode (primary / follower), TLS, and common configurations."
...
A follower is not a second login site. It only provides health checks and internal remote storage APIs. See [follower nodes](/en/admin/follower-nodes/) for the complete enrollment flow.
