---
description: "AsterDrive config.toml rate-limit fields: throttling rules for login, registration, WebDAV, and other endpoints, trigger behavior, and exemption boundaries."
title: "Rate Limiting"
---
...
Enforce strict global quotas at the Ingress, API gateway, or load-balancer layer. Application limits can remain as a second layer of per-Primary protection. See [Load Balancing and Multi-Instance Deployments](/en/deploy/multi-instance/#rate-limits-are-not-global-counters) for the complete entry contract.
