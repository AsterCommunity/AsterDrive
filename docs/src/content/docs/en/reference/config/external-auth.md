...
During migration, existing providers may still use `https://drive.example.com/api/v1/auth/external-auth/{kind}/{provider}/callback`. Register both URIs with the identity provider, confirm the unified URI works, then switch the callback mode in AsterDrive. Legacy compatibility will be removed in 1.0.0.
```
...
| --- | --- | --- |
| OIDC / Generic OAuth2 | Your identity provider admin console, such as the Applications / Clients page in Logto, Authentik, Keycloak, or Zitadel | The callback URL shown after saving the AsterDrive provider; new providers can pre-register `/api/v1/auth/external-auth/callback` |
