...
- **Find a security issue**: do not open a public issue first. Report it privately through the [security policy](https://github.com/AsterCommunity/AsterDrive/security/policy); automated tools can also discover the site's [`security.txt`](https://drive.docs.astercosm.com/.well-known/security.txt).
