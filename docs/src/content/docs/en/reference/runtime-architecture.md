---
description: "AsterDrive runtime architecture: primary and follower nodes, component relationships, multi-primary resource boundaries, upload and download data flows, the two configuration layers, and troubleshooting entry points."
...
If you do not know where to start, run `doctor` from [Operations CLI](/en/ops/cli/), then compare the specific symptom with [Troubleshooting](/en/ops/troubleshooting/).
