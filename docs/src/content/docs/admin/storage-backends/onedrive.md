...
大文件不会先完整写入某一台 Primary 的本地暂存文件。AsterDrive 会在初始化时创建 Microsoft Graph upload session，把每个浏览器分片按顺序流式转发为一个 `Content-Range` 请求，并把加密后的 provider session 状态保存在共享数据库中。多 Primary 部署下，初始化、后续分片、进度查询和完成请求可以落到不同 Primary，不需要 sticky session 或共享临时目录。

Graph upload URL 是预认证凭据。`server_relay` 模式不会把它返回给浏览器；浏览器只调用 AsterDrive 的分片接口。后端也会限制同一会话的分片并发为 1，并在 Graph 响应不明确时查询 provider 进度后再决定重试或推进。
...
Microsoft access token 和 refresh token 始终保留在 AsterDrive 服务端，不会发送给浏览器。两种方式都支持中断续传，取消或过期的上传也会自动清理；只有 `frontend_direct` 会把临时 Graph upload URL 交给已认证浏览器。
...
凭据加密依赖 `auth.storage_credential_secret_key`。备份或迁移 AsterDrive 时，需要同时保留这项配置，详见 [登录与会话 — `storage_credential_secret_key`](/reference/config/auth/#storage_credential_secret_key)。
...
先确认 OneDrive 策略里的下载方式选择为“Microsoft Graph 直接下载”。如果选择了“严格使用 AsterDrive 文件名”，重命名文件会使用代理流式下载；旧文件或浏览器无法访问 Microsoft 时，也可能使用代理流式下载。
