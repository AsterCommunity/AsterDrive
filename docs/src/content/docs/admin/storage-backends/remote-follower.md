---
description: "把已接入的 AsterDrive 从节点配置为远程存储策略后端的完整流程：远程存储目标、remote 策略、策略组规则、绑定用户或团队与上传下载验收。"
title: "远程节点存储策略教程"
---
...
如果你还没有把从节点接入主控，先看 [远程节点](/admin/follower-nodes/)。  
如果你用 Docker 跑从节点，先看 [Docker 部署从节点](/deploy/follower-node/)。
...
Follower 本身不是第二个 Primary，也不提供独立控制面。主控侧可以保持单 Primary，也可以按 [cluster 负载均衡契约](/deploy/multi-instance/)运行多个 Primary；两种形态都把 Follower 当作远程存储数据面。
...
要让公网用户访问这类文件，要么给 follower 提供公网可达的 HTTPS 地址，要么把远程策略的上传/下载方式改成 `relay_stream`，让主控节点代转流量。不同拓扑的取舍见 [从节点网络部署方式](/deploy/follower-node/network/)。
:::
