---
description: "把 AsterDrive 文件写入 S3 / MinIO / R2 对象存储的完整教程：准备 bucket、创建存储策略、配置策略组、绑定用户、presigned 直传 CORS 与验收。"
...
后台的 `Region` 字段对应存储策略 `options.s3_region`，用于 S3 SigV4 请求签名。它和 Endpoint、Bucket 是三个独立配置项，AsterDrive 不会从任意自定义 Endpoint 猜测 region。

- 首尾空白会被清理；留空或只包含空白时使用 `auto`，兼容未配置 region 的已有策略。
- 非空值必须为 1–128 个可打印 ASCII 字符，不能包含任何空白字符或 `/`。无效值会以 `BadRequest`（HTTP 400，响应码 `bad_request`）拒绝。
- AWS S3 应填写 bucket 实际所在 region，例如 `us-east-1`。
- Cloudflare R2 通常使用 `auto`。
- MinIO、RustFS 和其他 S3-compatible 服务应按服务商文档填写，例如 `us-east-1` 或服务商定义的值。
- 连接测试和保存后的运行时请求使用同一个 signing region。修改后应重新测试连接。
...
:::tip[华为云 OBS 的 CORS 字段]
Huawei Cloud OBS 使用原生 OBS 请求协议，不要机械复制上面示例中的 `x-amz-*` 请求头。OBS 预签名上传至少需要把 `Content-Type` 放进“允许的头域”；multipart 上传和 Range 下载需要把 `ETag`、`Content-Length`、`Content-Range`、`Accept-Ranges` 等响应头放进“补充头域”。如果 OBS 控制台中的 `OPTIONS` 预检返回 `403`，优先检查来源、`PUT` 方法和 `Content-Type` 是否匹配同一条 CORS 规则。
:::
