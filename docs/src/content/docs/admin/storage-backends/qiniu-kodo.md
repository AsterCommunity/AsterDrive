...
Endpoint 只接受 HTTPS，并使用 `https://s3.<region>.qiniucs.com` 服务级格式，例如 `https://s3.cn-east-1.qiniucs.com`；`<region>` 必须与表单中的 SigV4 region 相同。本 connector 不接受明文 HTTP、bucket-prefixed endpoint、自定义 CNAME、非标准端口或带 path/query/fragment 的 URL；其他 S3-compatible 服务应使用通用 S3 connector。
...
| 字段 | 说明 |
| --- | --- |
| Kodo S3 endpoint | 官方服务级 endpoint，例如 `https://s3.cn-east-1.qiniucs.com`，不含 S3 空间名。 |
...
| Path-style 寻址 | 默认开启，使用 `/S3-空间名/key`；关闭后使用 `S3-空间名.s3.<region>.qiniucs.com/key`。七牛官方支持两种方式。 |
...
连接失败时，按 endpoint 可达性、S3 空间名、region 是否匹配、path-style、AccessKey/SecretKey、权限和服务器时间的顺序检查。不要在 AsterDrive 日志、错误反馈或工单中粘贴 SecretKey 或完整签名 URL。

只有更换 S3 空间名、基础路径或实际目标存储位置时，才需要新建目标策略并用存储迁移任务搬迁已有 blob。对同一空间和基础路径，仅切换 path-style 寻址，或纠正与该空间匹配的 endpoint / region，不会改变对象 key，无需迁移；配置不匹配时直接修正配置。
