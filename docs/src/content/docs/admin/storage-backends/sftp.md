...
如果你需要浏览器直传，优先看 [S3 / MinIO / R2](/admin/storage-backends/s3/)、[Azure Blob Storage](/admin/storage-backends/azure-blob/) 或 [腾讯云 COS](/admin/storage-backends/tencent-cos/)。SFTP 的上传和下载都经过 AsterDrive 服务端。
...
覆盖已有对象时，SFTP 服务器需要支持 `posix-rename@openssh.com`。该扩展提供 POSIX 原子替换语义；服务器未声明扩展时，AsterDrive 保留旧对象并返回能力错误，不使用具有短暂缺失窗口的普通双 rename 作为替代。
