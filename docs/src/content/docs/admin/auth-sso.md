...
关闭后，登录页隐藏本地密码输入和登录按钮，`POST /auth/login` 也会拒绝本地密码第一因子。注册、邀请、密码重置、首次初始化、管理员设密、已登录改密和外部身份密码绑定流程仍然保留；已有账号、密码和会话不会被删除。关闭前请至少保留一个可用的外部 provider 或 Passkey 方式，并确认管理员仍能通过保留方式登录。
