---
description: "AsterDrive 自定义前端覆盖机制：前端资源加载顺序、index.html 占位符、用自定义配置做全局变量持久化层，以及 CSP 与安全限制。"
title: "自定义前端"
---
...
运维 CLI 也支持自定义配置——`list` / `get` / `set` / `delete` / `validate` / `export` / `import` 全部通用。详见 [运维 CLI](/ops/cli/)。
