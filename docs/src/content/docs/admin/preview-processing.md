---
description: "AsterDrive 预览与文件处理的管理员配置：三类打开方式、WOPI 接入顺序、公开站点地址、Discovery 导入、图片与压缩包预览开关、分享流播放会话、锁处理、CORS 和上线验收。"
title: "预览与文件处理"
---

:::tip[这一篇讲什么]
管理员视角：怎么给文件配置额外打开方式，尤其是把 Office 文件交给 OnlyOffice、Collabora 或其他 WOPI 兼容服务打开；以及图片、压缩包、音视频预览的后台开关。
普通用户怎么使用这些打开方式，见 [预览与编辑](/using/preview-editing/)。
:::
...
反向代理相关说明见 [反向代理](/deploy/reverse-proxy/#wopi--office-回调的额外要求)。
...
后端预览图第一次缺失时会创建 `image_preview_generate` 后台任务；任务完成前，页面可能会先显示加载或回退状态，后续再打开会复用缓存。
...
文件名编码选项（`自动`、`GB18030`、`CP437`、`Shift_JIS` 等）的用户侧说明见 [预览与编辑](/using/preview-editing/#压缩包只读预览)。
