...
single 可以在管理端创建 `local` 策略，例如把绝对路径设为 `/data/uploads`；cluster 同样可以使用 `local`，但部署者必须保证 policy 路径和 `upload_temp_dir` 对所有 Primary 共享，且文件系统提供跨实例可见性、`fsync`、原子 rename/delete 和 advisory lock 语义。完整要求见[存储与上传限制](/deploy/multi-instance/#存储与上传限制)。对象存储、SFTP 和远程 Follower 也必须由所有 Primary 访问。两种 profile 的 setup API、策略创建、默认组回填和 readiness 状态迁移完全相同。
...
12. 如果准备接远程节点，follower 是否已经完成 enroll、内部协议能力是否兼容，并且 remote 策略已显式绑定一个已应用的远程存储目标
