...
| `file`、`folder`、`team_service`、`share_service`、`trash_service`、`version_service` | 文件与工作空间 | 文件主链路、团队空间、分享、回收站和版本 |
| `upload_service`、`workspace::storage`、`policy_service`、`storage::*` | 上传与存储 | 上传会话、存储策略选择、Blob 写入和驱动抽象 |
| `thumbnail`、`media_processing_service`、`media_metadata_service`、`archive_preview_service`、`wopi_service` | 预览与处理 | 文件派生结果、在线打开和预览能力 |
| `config_service`、`task_service`、`mail::sender`、`audit_service`、`health_service`、`readiness_service` | 系统与运维 | 热配置、后台任务、邮件、审计和健康检查 |
