...
AsterDrive currently supports three provider kinds:
...
`{kind}` is `oidc`, `generic_oauth2`, or `github`, and `{provider}` is the provider key generated by the server.
...
| Issuer URL | Required for OIDC; optional for Generic OAuth2. For Generic OAuth2, it becomes the identity namespace if set. GitHub uses fixed endpoints and does not require this |
| Authorization URL | Required for Generic OAuth2; discovered for OIDC; fixed for GitHub |
| Token URL | Required for Generic OAuth2; discovered for OIDC; fixed for GitHub |
| UserInfo URL | Required for Generic OAuth2; OIDC currently primarily uses ID Token claims; GitHub reads `https://api.github.com/user` |
...
Generic OAuth2 uses `openid email profile` when scopes are blank. OIDC also uses `openid email profile` by default, and the server guarantees that `openid` is present; if you manually save OIDC scopes as `email profile`, `openid` is added back. GitHub uses this default when scopes are blank:
...
Microsoft Entra ID and Google should be configured as OIDC for sign-in. Do not add OAuth2 login providers for them just because their endpoints contain `/oauth2/`. OAuth2 access tokens are only needed later when AsterDrive needs to access Microsoft Graph, Google APIs, Google Drive, or similar protected resources.
...
The current generic OIDC driver strictly requires the ID token issuer to match the configured Issuer URL, so manual Microsoft configuration should prefer a concrete tenant ID. Multi-tenant entries such as `organizations`, `consumers`, and `common` need a dedicated preset to handle issuer / tenant semantics; see the tracking issue below. Microsoft does not always return an `email` claim, and its email claim should not be treated like GitHub's verified primary email semantics; if email is missing, use the existing email-verification / account-binding flow.
...
Google identities must be linked by `sub`, not by email. Google API / Drive access is a separate OAuth authorization feature and should not be mixed into the login provider's default scopes.

Microsoft preset tracking issue: <https://github.com/AptS-1547/AsterDrive/issues/263>. Google preset tracking issue: <https://github.com/AptS-1547/AsterDrive/issues/265>.
...
Auto-linking requires a usable email, `email_verified=true`, and exactly one local user with that email. The dedicated GitHub provider only treats email as verified when `/user/emails` returns a verified primary email; Microsoft-like OIDC providers may omit the email claim, in which case users may need email verification or password binding.
