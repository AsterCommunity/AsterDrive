---
description: AsterDrive 外部认证配置指南，覆盖 OpenID Connect、通用 OAuth2、Logto、GitHub、Google、账号绑定策略和故障排查。
---
...
如果提供商支持标准 OIDC，优先用 `oidc`。GitHub 和 Google 用专用 provider。只有在 provider 没有完整 OIDC discovery，或者你明确要手动接 OAuth2 userinfo 时，再用 `generic_oauth2`。
...
其中 `{kind}` 是 `oidc`、`generic_oauth2`、`github` 或 `google`，`{provider}` 是服务端生成的 provider key。
...
| Issuer URL | OIDC 必填；Generic OAuth2 可选。Generic OAuth2 填了以后会作为身份命名空间；GitHub 使用固定端点；Google 固定为 `https://accounts.google.com` |
| Authorization URL | Generic OAuth2 必填；OIDC 从 discovery 获取；GitHub 固定为 GitHub OAuth endpoint；Google 从固定 discovery 获取 |
| Token URL | Generic OAuth2 必填；OIDC 从 discovery 获取；GitHub 固定为 GitHub OAuth endpoint；Google 从固定 discovery 获取 |
| UserInfo URL | Generic OAuth2 必填；OIDC 当前主要使用 ID Token claims；GitHub 固定读取 `https://api.github.com/user`；Google 当前主要使用 ID Token claims |
...
Microsoft Entra ID 优先按 OIDC 接入，不要为了登录新增 OAuth2 provider。OAuth2 access token 只在后续需要访问 Microsoft Graph 等资源时才需要。
...
当前通用 OIDC driver 会严格要求 ID Token issuer 等于配置的 Issuer URL，因此手动配置 Microsoft 时优先使用具体 tenant ID。`organizations`、`consumers`、`common` 这类多租户入口需要专用预设处理 issuer / tenant 语义，跟踪 issue：<https://github.com/AptS-1547/AsterDrive/issues/263>。Microsoft 的 `email` claim 不保证一定存在，也不要默认套用 GitHub 的 verified primary email 语义；邮箱缺失时按现有邮箱补验 / 绑定流程处理。
...
自动绑定要求 provider 返回可用邮箱和 `email_verified=true`，并且本地只有一个同邮箱用户。GitHub 专用 provider 只在 `/user/emails` 返回已验证主邮箱时才会认为邮箱已验证；Google 专用 provider 使用 ID Token `email_verified`；Microsoft 这类 OIDC provider 的邮箱 claim 可能缺失，必要时让用户走邮箱补验或密码绑定。
