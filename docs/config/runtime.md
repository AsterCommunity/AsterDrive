...
| 后台任务、缩略图、压缩包预览、回收站保留策略异常 | 运行与调度 / 存储与保留 | 再看 [运维 CLI](/deployment/ops-cli) |
...
- **存储与保留** —— 回收站、历史版本、默认配额、任务产物、在线解压 staging、压缩包预览、媒体处理
- **WebDAV** —— 总开关、常见系统文件拦截
- **审计日志** —— 开关和保留时间
- **自定义配置**、**其他** —— 高级场景才碰
...
如果你把后台任务并发调大，在线压缩、在线解压、压缩包预览和缩略图任务会更容易同时跑起来，CPU、内存和 I/O 压力也会一起上来。
