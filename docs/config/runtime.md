...
链接导入会创建一个单独的后台任务车道，用来让服务端从 HTTP/HTTPS 源地址下载文件，再导入到当前工作空间。

这一组现在有这些关键设置：

- **链接导入文件大小上限**：服务端允许下载的最大源文件大小
- **链接导入下载速度上限**：单个任务的最大平均下载速度，前端按 MB/s 显示；填写 `0` 表示不限制
- **链接导入任务并发上限**：同时允许跑多少个链接导入任务
- **链接导入请求超时**：单次外部 HTTP/HTTPS 请求最多持续多久
- **链接导入引擎注册表**：按顺序启用 `builtin` 和 `aria2` 下载引擎；启用多个引擎时会按优先级尝试，全部关闭时链接导入会被禁用
- **aria2 RPC 地址 / 密钥 / RPC 超时**：只在 `aria2` 引擎启用时使用；RPC 超时是单次 JSON-RPC 调用超时，不替代完整下载超时
- **aria2 split / 单服务器连接数 / 最低速度阈值**：仅暴露这几个管理员控制的安全子集，不透传任意 aria2 参数

默认值按“能直接用、不会明显打扰现有行为”来设，并避免默认无限速：

- 链接导入引擎注册表：`builtin` 启用，`aria2` 关闭
- 文件大小上限：`1 GiB`
- 下载速度上限：`5` MB/s；填写 `0` 仍表示不限制
- 并发上限：`1`
- 请求超时：`600` 秒
- aria2 RPC 超时：`10` 秒
- aria2 split：`5`
...
链接导入只支持 HTTP/HTTPS，不跟随重定向，且会拒绝解析到本机、私网、链路本地、多播、文档保留地址和元数据地址的主机，避免把服务端当成内网探测器。内置引擎会把下载流式写入临时文件，不会把整文件塞进内存；文件下载完成后才会进入 SHA-256 校验和导入步骤。

当启用 `aria2` 引擎时，AsterDrive 仍会在派发任务前校验源 URL，但 aria2 守护进程会自行做 DNS 解析和出站连接。运维侧应在网络层隔离 aria2，并把 JSON-RPC 端点限制为只有 AsterDrive 可以访问。管理后台的链接导入引擎注册表里可以执行 aria2 RPC 探测，服务端会调用 `aria2.getVersion` 验证 RPC 地址、密钥和连通性。
...
| 链接导入引擎注册表、文件大小、速度、并发、请求超时和 aria2 参数 | 之后新建的链接导入任务按新规则判断 |
