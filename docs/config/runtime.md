...
| 登录 Cookie、Token、激活链接时效不合适 | 认证与 Cookie | 再看 [登录与会话](/config/auth) |
| 注册、头像、Gravatar 行为不符合预期 | 用户管理 | 再看 [管理后台](/guide/admin-console) |
| Passkey、外部登录、外部身份绑定不符合预期 | 站点配置 / 管理 -> 外部认证 / 认证与 Cookie | 再看 [登录与会话](/config/auth) |
...
| WebDAV 总开关或行为异常 | WebDAV | 再看 [WebDAV](/config/webdav) |
...
| 外部登录邮箱验证邮件模版 | 之后新发出的外部登录验证邮件按新规则 |
