...
链接导入默认只支持 HTTP/HTTPS，不跟随重定向，且会拒绝解析到本机、私网、链路本地、多播、文档保留地址和元数据地址的主机，避免把服务端当成内网探测器。下载过程是流式写入临时文件，不会把整文件塞进内存；文件下载完成后才会进入 SHA-256 校验和导入步骤。
...
| 链接导入文件大小、速度、并发和请求超时 | 之后新建的链接导入任务按新规则判断 |
