...
fn postgres_find_or_create_blob_insert_sql_uses_valid_on_conflict() {
    let now = Utc::now();
    let sql = aster_drive_model::entities::file_blob::Entity::insert(file_blob::ActiveModel {
...
fn summarize_blobs_by_policy_sql_uses_seaorm_aggregate_query() {
    let statement = aster_drive_model::entities::file_blob::Entity::find()
        .select_only()
...
fn find_active_blob_by_hash_sql_excludes_cleanup_claimed_rows() {
    let sql = aster_drive_model::entities::file_blob::Entity::find()
...
fn increment_blob_ref_count_sql_uses_cas_against_cleanup_claim() {
    let sql = aster_drive_model::entities::file_blob::Entity::update_many()
        .col_expr(
