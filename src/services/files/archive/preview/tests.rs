...
async fn bounded_copy_future_size_and_allocations_are_bounded() {
    ...
    assert!(
        allocations.count <= 2,
        "archive preview copy allocated {} objects; expected one buffer plus fixed control-plane overhead",
        allocations.count
    );
    assert!(
        allocations.bytes >= COPY_BUFFER_BYTES,
        "archive preview copy allocated only {} bytes; reusable buffer is missing",
        allocations.bytes
    );
    assert!(
        allocations.bytes <= COPY_BUFFER_BYTES + CONTROL_PLANE_ALLOCATION_BUDGET,
        "archive preview copy allocated {} bytes; control-plane overhead is not bounded",
        allocations.bytes
    );
}
