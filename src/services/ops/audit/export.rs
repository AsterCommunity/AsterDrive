...
fn parse_details(model: &audit_log::Model) -> Option<serde_json::Value> {
    let raw = model.details.as_deref()?;
...
async fn usernames_for_batch(
    ...
    for model in models {
        ...
        if let Some(id) = member_user_id(parse_details(model).as_ref()) {
            ids.insert(id);
        }
    }
    ...
    let stream = Box::pin(async_stream::try_stream! {
        ...
        if let Some(snapshot) = snapshot {
            ...
            loop {
                ...
                let usernames = usernames_for_batch(&state, &models)
                    .await
                    .map_err(|error| {
                        tracing::error!(
                            export_kind = kind.log_name(),
                ...
                let chunk = csv_chunk(rows_for_batch(&models, &usernames))?;
