...
impl std::fmt::Debug for StorageConnectorCredentialInput {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self {
            Self::None => "None",
            Self::Static(_) => "Static(<redacted>)",
            Self::AuthorizationApplication(_) => "AuthorizationApplication(<redacted>)",
        })
    }
}
...
#[cfg_attr(all(debug_assertions, feature = "openapi"), derive(ToSchema))]
pub struct StorageConnectionInput {
    pub connector_config: ConnectorConfigEnvelope,
    pub credential: StorageConnectorCredentialInput,
}

/// Full connection input for a storage policy. Remote targets use the same
...
pub struct StoragePolicyConnectionInput {
    #[serde(flatten)]
    pub storage: StorageConnectionInput,
