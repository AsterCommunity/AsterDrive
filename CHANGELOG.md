...
- **Admin detail workflows** — User, team, storage policy, and external authentication provider records now open dedicated, directly addressable detail pages that share consistent back navigation, page headers, responsive summaries, and reduced-motion-aware entrance transitions. User profile, policy-group, and security settings use a staged two-column editor; team overview, members, audit, and danger tabs retain URL navigation and independent scrolling; saved storage policies place identity, vertically stacked capacity, and basic fields in a sticky summary column beside descriptor-driven connection, credential, authorization, action, promotion, and upload-rule controls, with required schema fields consistently marked. Multi-step storage-policy and external-provider creators use `/admin/policies/new` and `/admin/external-auth/new`, guard unsaved navigation, and open the created record page; external-provider details keep configuration and sign-in policy in a sticky summary beside identity, connection, and claim controls. Creating users, inviting users, and creating teams remain focused dialogs on their list pages.
