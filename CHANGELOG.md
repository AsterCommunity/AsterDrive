...
- **Operator-managed filesystem storage in clusters** — The existing `asterdrive.storage.local` connector is now `deployment_managed` and may be used by multi-Primary deployments without introducing a duplicate shared-filesystem connector. Operators are responsible for mounting every policy path and `server.upload_temp_dir` as shared filesystems with the required cross-instance visibility, durability, atomic rename/delete, and advisory-lock semantics. Staged uploads retain database receipts, completion claims, leases, cleanup, and retry behavior, while concurrent completion conflicts now return HTTP 409 with the stable `upload.status_conflict` code.

- **Capacity-aware upload placement** — Upload initialization now classifies target capacity as sufficient, insufficient, unsupported, or unavailable before creating sessions or provider-side state. Conclusively insufficient and temporarily unavailable targets participate in the existing placement fallback rules, while connectors without a portable capacity API skip runtime probes and rely on the data-plane result. Demand-driven capacity probes are coalesced per policy, globally bounded, timeout-limited, and independent from request cancellation; drivers own cost-appropriate cache windows, OneDrive and Remote expose a bounded probe-timeout setting, stale sufficient observations use stale-while-revalidate, and stale rejection decisions are confirmed before returning an error. Exhausted candidates return distinct stable 507 or retryable 503 errors; upload data-plane, cache, probe latency, and capacity outcomes are observable without high-cardinality labels.

- **Physical upload-staging reservations** — `OffsetStaging` and `StreamStaging` now serialize temporary-filesystem admission and physically allocate the full declared size before Init succeeds, preserving a configurable 256 MiB free-space floor. Insufficient staging capacity returns a stable 507 without leaving session or directory side effects; active sparse reservations are rebuilt on the first staged Init, Chunk PUT, or Complete after restart, and existing completion/cancel/expiry cleanup releases the allocated blocks.
...
- **Connector-defined download delivery URLs** — Storage connectors can declare descriptor-driven custom download domains and keep their configuration in the connector-owned policy schema. Tencent COS policies can use an optional HTTPS `download_base_url`; authenticated file downloads and `resource-handle` initialization share one direct-delivery resolver that signs the final host, returns URL lifetime and browser credential policy, preserves attachment/inline semantics, and falls back to same-origin streaming when direct delivery is unavailable. Uploads, cleanup, bucket administration, public-share limits, and backend object I/O remain on their existing controlled paths.

- **Storage policy disaster recovery** — Deleting a policy with blob references now opens an administrator recovery workflow backed by bounded read-only object probes. Readable and virtual-empty content can be migrated through the existing fenced background-task system while the source policy remains for an explicit later decision; unrecoverable remnants can be permanently purged only after a fresh impact preview and exact confirmation phrase, preserving canonical file/version/share/trash/quota deletion semantics. The storage-migration request removes `delete_source_after_success`; clients must use `mode` and, for `recover_available`, the latest `recovery_plan_hash`. Ordinary policy deletion and upload cleanup protections remain unchanged.

- **Storage placement policy engine** — Policy groups upgraded to placement profiles with versioned upload admission, ordered matchers, `first_available` / `weighted_random` target selection, per-target new-file intake control, unavailable fallback, and upload execution preferences; file-category admission is unrestricted by default and uses a single optional allowlist instead of competing allow/deny lists. The legacy item migration preserves original size routing, and user/team assignment, folder override, existing blob policy, and upload session decision boundaries remain unchanged. New blob ingress uniformly reuses the immutable `PolicySnapshot` resolver; the admin side supports rule/target editing and backend dry-run simulation, showing normalized categories, admission results, rule traces, excluded targets, and stable reason codes.
...
- **Admin detail workflows** — User, team, storage policy, external authentication provider, and remote node records now open dedicated, directly addressable detail pages that share consistent back navigation, page headers, responsive summaries, and reduced-motion-aware entrance transitions. User profile, policy-group, security, and remote-node settings use staged two-column editors; remote-node details split basic information and remote storage targets into URL-driven subpages with D9 line tabs and local add/remove transitions; team overview, members, audit, and danger tabs retain URL navigation and independent scrolling; saved storage policies place identity, vertically stacked capacity, and basic fields in a sticky summary column beside descriptor-driven connection, credential, authorization, action, promotion, and upload-rule controls, with required schema fields consistently marked. The storage-policy creator remains multi-step at `/admin/policies/new`; external-provider and remote-node creators use page-level workflows at `/admin/external-auth/new` and `/admin/remote-nodes/new`, opening the created record page, with remote-node creation handing off a one-time enrollment command before opening it. Editing an existing policy group keeps the user on its detail page after saving without replaying local rule animations; external-provider and remote-node details keep summaries and diagnostics beside editable configuration. Creating users, inviting users, and creating teams remain focused dialogs on their list pages.

- **Connector-owned icon assets** — Storage connectors now compile and expose their own icon bytes through a versioned same-origin backend endpoint; descriptor clients consume structured URL, media type, and revision metadata instead of frontend-owned static paths or core icon-library names. Versioned requests use immutable caching, while unversioned requests revalidate before reuse.

- **Unified storage connection and remote target RPC** — Regular storage policies and follower remote storage targets share the same `StorageConnectionInput`, connector config/credential normalization, descriptor, credential retention semantics, and direct driver factory; the remote target is additionally responsible for binding, target key, revision, reconciliation, and signed RPC, while every remote policy selects its target explicitly and follower requests without a target key are rejected. A remote policy's node, target, and base path become immutable after creation; changing physical location requires a new policy and policy-to-policy storage migration. The primary rejects deleting or changing the connector configuration of a target while any policy references the same node and target key, while display-name edits, credential-only rotation, and policy transfer-strategy changes remain available. Target editing keeps the connector type immutable while allowing same-connector configuration and credential updates within the existing reference-safety boundary. Field descriptors expose `mutable`, `create_only`, and `set_once` update behavior so generic clients render these boundaries without connector or field-name matrices, while backend services independently enforce them for direct API callers. Local, S3, SFTP, Tencent Cloud COS, Alibaba Cloud OSS, Qiniu Kodo, Azure Blob, and Huawei OBS are all wired in through a single runtime connector registry, and the admin side reuses the ordinary storage descriptor-driven field forms; policy creation can select or quickly add a target, while saved policy details no longer duplicate the remote node's target list and target management stays on the remote node detail page, using the same frameless table language as policy management with independent target actions instead of mixing them with remote-node save/test actions; target forms reuse descriptor conditional-required/default rules, and target-list loading no longer blocks an already-open create flow. Internal protocol V6 negotiates the follower's remote target connector IDs and exposes the follower-owned descriptor and localization catalog through the signed control plane, so a primary may manage connectors that it does not itself register; the unreleased schema cleanup removes binding-wide default target state and its index.
