...
- **媒体处理与缩略图**
  ...
  - 存储策略新增 `storage_native_thumbnail_enabled`、`storage_native_thumbnail_extensions`、`storage_native_media_metadata_enabled` 与 `storage_native_media_metadata_extensions`，支持按扩展名启停存储原生缩略图和媒体信息解析；关闭只停用对应的原生候选，文件仍继续经过全局媒体处理器
- **管理后台**
...
- **存储策略配置**
  ...
  - `storage_native_thumbnail_extensions` 与 `storage_native_media_metadata_extensions` 在对应开关为 `false` 时作为休眠配置保留，重新启用可恢复原匹配规则
- **API 错误结构**
