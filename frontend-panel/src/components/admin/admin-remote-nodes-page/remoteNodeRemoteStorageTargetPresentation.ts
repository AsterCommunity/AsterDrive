...
export function getRemoteNodeRemoteStorageTargetProfileStatus(
	...
	if (!profile.connector_available)
		return {
			labelKey: "remote_node_ingress_profile_status_unavailable",
			toneClass:
	...
	if (profile.last_error.trim())
		return {
			...
			toneClass: "border-destructive/50 bg-destructive/10 text-destructive",
		};
...
export function getRemoteNodeRemoteStorageTargetConnectorBadgeTone(
	available: boolean,
) {
	return available
