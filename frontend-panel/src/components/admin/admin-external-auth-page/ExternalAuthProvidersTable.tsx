...
interface ExternalAuthProvidersTableRowProps {
	deletingId: number | null;
	onCopyCallbackUrl: (value: string) => void;
	onEdit: (provider: AdminExternalAuthProviderInfo) => void;
	onRequestDelete: (id: number) => void;
...
export function ExternalAuthProvidersTableRow({
	...
	providerKinds,
	testingId,
}: ExternalAuthProvidersTableRowProps) {
	const { t } = useTranslation("admin");
		...
		>
			<TableCell>
				<div className={ADMIN_TABLE_TEXT_CELL_CLASS}>
					<span className={ADMIN_TABLE_MONO_TEXT_CLASS}>{provider.id}</span>
				</div>
			</TableCell>
			<TableCell>
				<div className={ADMIN_TABLE_TEXT_CELL_CLASS}>
				...
				<div className={ADMIN_TABLE_STACKED_CELL_CLASS}>
					...
					>
						{primaryEndpoint
							...
							: t("external_auth_provider_primary_endpoint")}
						{" · "}
						{t("core:updated_at")}: {formatDateAbsolute(provider.updated_at)}
					</span>
				</div>
			</TableCell>
			<TableCell>
			...
			<TableCell>
				<div className={ADMIN_TABLE_BADGE_CELL_CLASS}>
					<Badge variant="outline" className={providerStatusTone(provider)}>
						{provider.enabled
							? t("external_auth_provider_enabled_badge")
							: t("external_auth_provider_disabled_badge")}
					</Badge>
					<Badge variant="outline">{securityModeLabel(t, provider)}</Badge>
